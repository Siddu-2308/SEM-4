...
/* Takes as input an array and two boundary indices. 
	Generates a pivot element for the sub-array given by the start_index and end_index. 
	Swaps the pivot element with the element with the element given by the end_index */ 
...
/* 
	Takes as input an array and two boundary indices. 
	Partitions the sub-array given by the boundary indices based on a pivot and returns the partition index. 
	That is, 
		all the elements from the left boundary to the partition index are less than or equal to the pivot, 
		and all the elements from (partition index+1)-th location to the right boundary are greater than pivot. 
	This pivot should be generated by calling the get_pivot() function.
*/
	int pivot = get_pivot(array, start_index, end_index);
	swap(array, pivot, start_index);
	...
	while(left < right){
		while(array[left] <= array[start_index])
			left++;
		while(array[right] > array[start_index])
			right--;
...
/* 
	Takes as input an array and two boundary indices. 
	Returns the subarray given by the boundary indices sorted in the non-decreasing order. 
	Implement this using the function partition and making recursive calls to itself
*/
